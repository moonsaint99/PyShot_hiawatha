...
def ref_trans_array(mat1, mat2, theta1): #mat = [rho, vp, vs]
...
def refl_optimize_kernel(lowermat, theta, minmax):
    ice_rho = 917
    ice_vp = 3840
    ice_vs = 1930
    return minmax*ref_trans_array([ice_rho, ice_vp, ice_vs], lowermat, theta)


def reflectivity_optimizer(mat1min, mat1max, theta, optimum):
    ...
    optimum = sp.optimize.minimize(refl_optimize_kernel, np.array([mat1_rhomin, mat1_vpmin, mat1_vsmin]), args=(theta, minmax),
                                   bounds=[(mat1_rhomin, mat1_rhomax), (mat1_vpmin, mat1_vpmax), (mat1_vsmin, mat1_vsmax)])

    return optimum.x


def reflectivity_window(mat1min, mat1max, theta_array):
    max_refl = []
    min_refl = []
    for theta in theta_array:
        max_params = reflectivity_optimizer(mat1min, mat1max, theta, 'max')
        max_refl.append(-1*refl_optimize_kernel(max_params, theta, -1))
        min_params = reflectivity_optimizer(mat1min, mat1max, theta, 'min')
        min_refl.append(refl_optimize_kernel(min_params, theta, 1))
    return np.array(max_refl), np.array(min_refl)


def reflectivity_array(mat, theta_array):
    """
    ...
    reflectivity_array = []
    for theta in theta_array:
        reflectivity_array.append(refl_optimize_kernel(mat, theta, 1))
...
def shuey_kernel(lowermat, theta, minmax):
    ice_rho = 917
    ice_vp = 3840
    ice_vs = 1930
...
def shuey_array(mat1, theta_array):
    """
    ...
    reflectivity_array = []
    for theta in theta_array:
        reflectivity_array.append(shuey_kernel(mat1, theta, 1))
    return np.array(reflectivity_array)